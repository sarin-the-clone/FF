...
###### using Prophet #####
##############################################################################################################
start = df_t.index[int(src.year_train*365)] 
end =  df_t.index[-src.dd_horizon-1]
for today in pd.date_range(start,end,freq='2W'):
    ind_train = pd.date_range(df_t.index[0],today)
    ind_curr = pd.date_range(today-timedelta(dhist-1),today)
    ind_pred = pd.date_range(today+timedelta(1),today+timedelta(dhoriz))
    
    # generates prophet model:
    src = lol.weather_station(df_t.loc[ind_train], dhist, dhoriz, n_years_2train, 'prophet')
    yhat = src.ph_predict()
    yhat.loc[ind_train] = df_t.loc[ind_train].copy()

    plt.figure(1),plt.cla()
    df_t.loc[ind_curr].plot(style='b-')
    df_t.loc[ind_pred].plot(style='g-')
    yhat[ind_pred].plot(style='r--')
    ...
    plt.figure(2),plt.cla()
    df_aGDD_s.loc[ind_curr].plot(style='b-')
    df_aGDD_s.loc[ind_pred].plot(style='g-')
    yhat[ind_pred].plot(style='r--')
    plt.title(('today: '+str(today.date()), 'HT: ' + str(Harvest), 'HT_hat: ' + str(Harvest_hat),
                '(today - HT,HT_hat - HT): (' + str((Harvest - today.date()).days) + ',' + str((Harvest_hat - Harvest).days) + ')' ))
